...
export function SihCatalogClient({ facets }: { facets: SihFacets }) {
  ...
  const activeTags = useMemo(() => {
    ...
    return tags;
  }, [search, categories, weapons, rarities, exteriors, priceMin, priceMax, statTrak, souvenir]);
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="tnum text-sm text-[color:var(--color-text-secondary)]">
              {data ? `${data.total.toLocaleString()} skins` : "…"}
            </span>
          </div>
          <select
            value={sort}
        ...
        {activeTags.length > 0 && (
          <div className="mb-4 flex flex-wrap items-center gap-1.5">
            ...
            <button onClick={clearAll} className="ml-1 text-xs text-[color:var(--color-accent)] hover:underline">
              Clear all
            </button>
          </div>
        )}
